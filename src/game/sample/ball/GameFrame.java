...
public class GameFrame extends JFrame {
	...
	private void doRendering(Graphics2D g2d, GameState state) {
		...
		for (int i = 0; i < state.sun.MyX.size(); i++) {
			...
			g2d.drawImage(state.sun.getImage3(), state.sun.MyX.get(i), state.sun.MyY.get(i), 100, 80, null);
		}
		...
		for (int i = 0; i < state.shooter.availableX.size(); i++) {
			...
			g2d.drawImage(state.shooter.getImage3(), state.shooter.availableX.get(i), state.shooter.availableY.get(i), 100, 80, null);
		}

		if(state.nz.speed ){
			for(c = 0 ; c<=state.nz.counter ; c++)

				g2d.drawImage(state.nz.getnZombie(),  state.nz.zx[c], state.nz.zY[c], 100, 80, null);
			}
